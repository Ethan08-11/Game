-- 销售+采购加厚：去掉基础破盾后盾容易挡满。掷点以 Java 为准。可重复执行。
UPDATE `bullies`
SET `bully_name` = '点名恶霸',
    `description` = '专挑更弱的护卫下手。血量持平时打销售。盾挡完仍会漏 10 点。',
    `hp` = 208, `attack_power` = 24, `defense_value` = 0, `status` = 1,
    `skill_data` = '{"customerCode":"CUSTOMER_KIND","pattern":"FOCUS_LOW_HP","chance":100,"shield":0,"bonusAttack":0,"catalogSummary":"专打更弱的护卫；盾挡完仍会漏 10 点。"}'
...
UPDATE `bullies`
SET `bully_name` = '针对恶霸',
    `description` = '仍打两人。约八成五回合盯打得最疼的人多挨 8 点。',
    `hp` = 208, `attack_power` = 24, `defense_value` = 0, `status` = 1,
    `skill_data` = '{"customerCode":"CUSTOMER_ANXIOUS","pattern":"FOCUS_TOP_DAMAGE","chance":85,"shield":0,"bonusAttack":8,"catalogSummary":"约八成五回合会盯打得最疼的人多挨 8 点。"}'

    ...
    <div class="rules-content">
      ...
      <div class="rules-body">
        ...
        <section class="rule-section">
          <h3>基本流程</h3>
          <ul>
            <li>每人牌库尽量凑满 30 张。当前用本部门成员卡：基础每种 2 张 + 本部门收藏各 1 张，再用公共部基础卡补位（雇佣兵与物流骑士）。中立卡已下架。不含对方部门。解锁不足时可少于 30 张，够了就必须满 30 张</li>
            <li>开局双方查看手牌后，由房主决定谁先出牌</li>
            <li>每回合从牌堆抽满 5 张手牌</li>
            <li>玩家轮流打出卡牌，消耗调用机会；结束回合时手中剩余牌进入弃牌</li>
            <li>双方都结束后，硬扛恶霸同时打两名护卫。面板目标显示部门，不显示 P1 / P2</li>
        ...
        <section class="rule-section">
          ...
          <ul>
            ...
            <li>各组队霸凌者血量均为 150。与玩家一样用血值；将其打到 0 即可救下顾客</li>
            <li>每回合双方出完牌后，硬扛恶霸出手：各组队攻击均为 17～20。约八成五打满、约一成五自己 +14 盾且攻击减半但仍出手。第 2 回合起顾客可能再加减攻击。护盾可以挡住</li>
        ...
        <section class="rule-section">
          <h3>复活</h3>
          <ul>
            <li>一名玩家倒下且队友仍存活时，可观看广告复活。全队每局只允许一名玩家复活，本局仍按最终胜负计入排行榜</li>
            <li>复活后血值在 16～22 点之间随机，且不超过自身上限</li>
            <li>若两名玩家都复活，立即判定保护失败，并计入败场。超时未复活、主动放弃或双方同时倒下，同样保护失败</li>
          </ul>
        </section>
        <section class="rule-section">
          <h3>对局奖励</h3>
        ...
        <section class="rule-section">
          <h3>账号与组队</h3>
          <ul>
            <li>同一账号同一时间只能在一端登录。其他端登录后，本端会立即下线</li>
            <li>不能与同一网络下的账号组队。本地开发双窗口测试不受此限制</li>
            <li>每天 0 点按总榜金币锁定前五名。与今日前五组队时，本局只会抽到「胆小怕事」或「焦虑难安」（焦虑约七成、胆小约三成），抽中后本局不变</li>
          </ul>
        </section>
        <section class="rule-section">
          <h3>每日任务与离开对局</h3>
          <ul>
            ...
            <li>每月固定扣 4 天单休：只有「当月天数 − 4」天能领金币（例如 10 月 31 天则 27 天）。2026 年 9 月从 12 日起算，全员只算 16 天。领满后天数为休息日，不发金币（含周常）</li>
            <li>主动放弃或掉线超时：记失败，占用当日第 N 局，不算完成、不发任务金币</li>
            <li>对局卡死且重连不上：离开菜单可申请「对局异常」作废本局（约 5 分钟无有效操作），不占用任务局数</li>
          </ul>
        </section>
      </div>
    </div>
